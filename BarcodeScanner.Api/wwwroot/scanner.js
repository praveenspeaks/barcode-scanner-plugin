...
class BarcodeScannerPlugin {
   ...
   * @param {string} options.apiUrl - Base URL of BarcodeScanner.Api
   ...
   * @param {number} [options.scanInterval=600] - ms between auto-scans in live mode
   * @param {function} [options.onResult]  - callback(result) fired when barcodes found
   * @param {function} [options.onError]   - callback(error)
   * @param {function} [options.onFrame]   - callback() fired on every scan attempt (for UI updates)
   */
  constructor(options = {}) {
    ...
    this.scanInterval = options.scanInterval ?? 600;
    ...
    this._canvas      = null;
    this._video       = null;
    this._isBusy      = false;   // prevents overlapping API calls
  }
  ...
  async scanImage(imgOrUrl) {
    ...
    return this.scanBase64(canvas.toDataURL('image/png'));
  }
  ...
  async startCamera(facingMode = 'environment') {
    ...
    // Wait for video to actually have dimensions before starting scans
  ...
  async _scanFrame() {
    // Skip this tick if the previous API call is still in-flight
    if (this._isBusy) return;

    // Skip if video hasn't loaded yet
    if (!this._video.videoWidth || !this._video.videoHeight) return;

    this._isBusy = true;
    try {
      const result = await this._doScan();
      if (this.onFrame) this.onFrame(result);
      if (result && result.success && result.barcodes.length > 0 && this.onResult) {
  ...
  async _doScan() {
    ...
    const base64 = this._canvas.toDataURL('image/jpeg', 0.9);
